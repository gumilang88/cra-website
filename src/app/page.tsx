...
function WeirdStats() {
  const stats = [
    { num: "100%", label: "Hair loss", Icon: PersonStanding },
    { num: "0%", label: "Willpower", Icon: BatteryLow },
    { num: "-99%", label: "Chart acc.", Icon: TrendingDown },
    { num: "∞", label: "Copium", Icon: Pill },
  ];
  return (
            ...
            <Runaway key={s.label} className="chaos-wander" style={{ ["--i" as string]: String(i) }}>
              <div className="flex min-w-[120px] flex-1 basis-[140px] items-center justify-center gap-2.5 rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-3 backdrop-blur-xl">
                ...
                <div className="text-left">
                  ...
                  <div className="mt-1 font-mono text-[10px] uppercase tracking-[0.14em] text-white/45">{s.label}</div>
                </div>
              </div>
            </Runaway>
          ))}
